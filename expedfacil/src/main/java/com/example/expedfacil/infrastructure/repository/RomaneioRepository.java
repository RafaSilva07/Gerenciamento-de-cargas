...
public interface RomaneioRepository extends JpaRepository<Romaneio, Long> {
    ...
    @Query("SELECT r.id AS id, r.numeroEmbarque AS numeroEmbarque, r.transportadora AS transportadora, r.motorista AS motorista, r.placa AS placa FROM Romaneio r")
    List<RomaneioResumoProjection> listarResumo();
